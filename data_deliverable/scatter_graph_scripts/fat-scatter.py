...
for count in countries:
    # print(count)
    c.execute("SELECT fat_c_pp_pd, year FROM dietary_compositions_by_commodity_group WHERE country = ? AND year BETWEEN 2000 and 2013 ORDER BY year ASC", (count,))
    lst_tuples = c.fetchall()
    fat_kc = [s[0] for s in lst_tuples]
    years = [s[1] for s in lst_tuples]
    plt.plot(years, fat_kc, label = count, linestyle= '-')
