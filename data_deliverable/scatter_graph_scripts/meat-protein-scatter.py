...
for count in countries:
    # print(count)
    c.execute("SELECT meat_protein_c, year FROM daily_caloric_supply_derived_from_carbohydrates_protein_and_fat WHERE country = ? AND year BETWEEN 2000 and 2013 ORDER BY year ASC", (count,))
    lst_tuples = c.fetchall()
    meat_protein_c = [s[0] for s in lst_tuples]
    years = [s[1] for s in lst_tuples]
    plt.plot(years, meat_protein_c, label = count, linestyle= '-')
